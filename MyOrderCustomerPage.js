...
export default class MyOrderCustomerPage extends React.Component {
  constructor(props){
    super(props);
    this.onAsyncRead();
    this.state = {
  ...
  onAsyncRead=async ()=>{
    ...
    database.readOrder(account.Email+':'+account.Phone, 'Order', this.readOrder_success);
  }
  render() {
      return (
      <View style={AllStyle.container}>
        ...
        <View style={this.styles.view1}>
          <TouchableOpacity onPress={()=>this.onRe()} style={{backgroundColor: (this.state.pushBtn==0)?'#FF2D55':'#31343D',borderRadius:100,width:'30%',height:'100%',alignItems:"center",justifyContent:"center",}}>
            <Text style={this.styles.text1}>รอการจัดส่ง</Text>
          </TouchableOpacity>
          <View style={{flex:0.2}}/>
          <TouchableOpacity onPress={()=>this.onCan()} style={{backgroundColor: (this.state.pushBtn==1)?'#FF2D55':'#31343D',borderRadius:100,width:'30%',height:'100%',alignItems:"center",justifyContent:"center",}}>
            ...
            <Text style={this.styles.text1}>กำลังจัดส่ง</Text>
          ...
          <TouchableOpacity onPress={()=>this.onCom()} style={{backgroundColor: (this.state.pushBtn==2)?'#FF2D55':'#31343D',borderRadius:100,width:'30%',height:'100%',alignItems:"center",justifyContent:"center"}}>
            <Text style={this.styles.text1}>ส่งแล้ว</Text>
          </TouchableOpacity>
          <View style={{flex:0.2}}/>
        </View>

        <View style={{flex:0.78,marginLeft:15,marginRight:15}}>
          <View style={{width:'100%',flexDirection:'row'}}>
            <View style={{flex:0.5,justifyContent:'center'}}>
              <Text style={this.styles.text2}>รายการคำสั่งซื้อ</Text>
            </View>
          </View>
          <View style={{flex:1}}>
            <FlatList
              ...
              renderItem={({ item }) => (item.Mode=='Loadmore')
                                      ? <View style={{justifyContent:'center',alignItems:'center',margin:25}}>
                                          <TouchableOpacity style={this.styles.btn1} onPress={()=>this.onPressLoadmore()}>
